...
async function sendPlayTimeUpdate(
  token: string,
  game_id: number,
...
async function checkUserAuthentication(): Promise<boolean> {
  try {
    const currentToken = await mainWindow.webContents.executeJavaScript(
...
function startSessionMonitoring() {
  ...
  sessionMonitoringInterval = setInterval(async () => {
    try {
      ...
      const currentSessionId = await mainWindow.webContents.executeJavaScript(
  ...
  if (activeGameSession) {
    ...
    try {
      ...
      const finalTotalPlayTimeMinutes =
        activeGameSession.lastRecordedPlayTimeMinutes + sessionDurationMinutes

      // Send final playtime update
      await sendPlayTimeUpdate(
        activeGameSession.token,
        activeGameSession.game_id,
            ...
            try {
              ...
              if (!isAuthenticated) {
                ...
                const sessionDurationMinutes =
                  (endTime - activeGameSession.sessionStartTime) / (1000 * 60)
                const finalTotalPlayTimeMinutes =
                  activeGameSession.lastRecordedPlayTimeMinutes + sessionDurationMinutes
                ...
                if (process.platform === 'win32') {
                  exec(`taskkill /pid ${gamePid} /f /t`, (err) => {
                    if (err) {
                      ...
                      console.log(
                        `Successfully terminated game process ${gamePid} due to authentication failure`
                      )
                    }
                  })
                } else {
              ...
              const sessionDurationMinutes =
                (currentTime - activeGameSession.sessionStartTime) / (1000 * 60)
              const newTotalPlayTimeMinutes =
                activeGameSession.lastRecordedPlayTimeMinutes + sessionDurationMinutes
              const updateSuccess = await sendPlayTimeUpdate(
                activeGameSession.token,
                activeGameSession.game_id,
              ...
              if (!updateSuccess) {
                ...
                if (process.platform === 'win32') {
                  exec(`taskkill /pid ${gamePid} /f /t`, (err) => {
                    if (err) {
                      console.error(
                        'Error killing game process due to auth failure in playtime update:',
                        err
                      )
                    } else {
                      console.log(
                        `Successfully terminated game process ${gamePid} due to playtime update authentication failure`
                      )
                    }
                  })
                } else {
              ...
              console.log(
                `Game process ${activeGameSession.pid} no longer running. Stopping periodic updates.`
              )
              const endTime = Date.now()
              const sessionDurationMinutes =
                (endTime - activeGameSession.sessionStartTime) / (1000 * 60)
...
app.whenReady().then(() => {
  ...
  autoUpdater.on('error', (error) => {
    ...
    if (error.message.includes('403') || error.message.includes('AuthenticationFailed')) {
      ...
      showMessage(
        'Update check failed: Authentication error. Please check GitHub token configuration.'
      )
    } else if (error.message.includes('404') && error.message.includes('github.com')) {
      showMessage(
        'Update error: Could not access GitHub repository. Please ensure you have the correct repository configuration.'
      )
    } else {
