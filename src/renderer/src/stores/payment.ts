...
const usePayment = defineStore('payment', {
  ...
  actions: {
    async reqTransaction(
      ...
      url: string = `${import.meta.env.VITE_APP_BASE_URL}/postorder/`,
      subtotal: number,
      currency: string = 'CLP'
    ): Promise<boolean> {
      this.loading = true
    ...
    async checkOrder(token: string): Promise<boolean> {
      try {
        ...
        switch (data.status) {
          ...
          case 'REJECTED':
            this.error =
              'The payment was rejected. Please check your payment details and try again.'
            return false
    ...
    async claimFreeGame(gameId: number, userId: number, auth: { token: string }) {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_API_URL}/v1/payment/claim-free-game`,
    ...
    async claimFreeGameDiscount(
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_APP_API_URL}/v1/payment/claim-free`,
