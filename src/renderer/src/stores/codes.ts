...
const useCodes = defineStore('codes', {
  ...
  actions: {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          this.userDiscountCodes = this.userDiscountCodes.filter(
            (assignment: UserHasDiscountCode) =>
              !(assignment.user_id === userId && assignment.discount_code_id === discountCodeId)
          )
          return { success: true, message: response.data.message }
        }
    ...
    async assignDiscountCodeToUserByLink(code: string, userId: number) {
      ...
      try {
        const response = await axios.post(
          '/v1/discount-codes/user/validate/',
          {
            user_id: userId,
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          return {
            success: true,
            data: response.data,
            message: 'Discount code successfully claimed!'
          }
        }

        return {
          success: false,
          message: response.data.message || 'Failed to claim discount code.'
        }
      } catch (error: any) {
        console.error('Error assigning discount code to user:', error)
