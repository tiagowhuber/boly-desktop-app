...
async function ApplyGameChanges() {
  try {
    ...
    const response = await fetch(
      import.meta.env.VITE_APP_API_URL + '/api/v1/games/' + game.value.id + '/updateData',
      {
        method: 'PATCH',
        headers: {
    ...
    if (response) {
      ...
      modalWarning.value = 'Error: No response from server'
    }
  } catch (error) {
    console.log(error)
  }
...
async function UploadFiles() {
  ...
  try {
    const response = await fetch(
      import.meta.env.VITE_APP_API_URL + '/api/v1/games/' + game.value.id + '/updateImages',
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${auth.token}`
    ...
    await fetch(
      import.meta.env.VITE_APP_API_URL + '/api/v1/games/' + game.value.id + '/deleteMedia',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
<template>
  <div v-if="auth.verifying || loading" class="loading_container">
    <Loading />
  </div>
  <div v-else class="section">
    ...
    <div class="mform">
      <div class="input-container">
        <h2>{{ $t('game_data') }}</h2>
        <LocaleSelector @language-changed="changeLocale" />
      </div>
      <div class="input-container">
        <div class="fullsize">
          <br />
        ...
        <div class="input-container">
          ...
          <TagAbove class="halfsize">
            ...
            <template #child>
              ...
              <input id="background" type="file" @change="handleBackgroundUpload" />
            </template>
          </TagAbove>
        </div>
        <br />
            ...
            <div class="media-list">
              ...
              >
                ...
                <div class="xmark" @click="SetDeleteMedia(item)">
                  ...
                  <p v-if="deleteMediaFiles.includes(item)">
                    Marcado para eliminar (Clic para cancelar)
                  </p>
                  <p v-else>Quitar archivo multimedia</p>
                </div>
              </div>
            </div>
            <input id="media" type="file" multiple @change="handleMediaUpload" />
          </template>
        </TagAbove>
        <br />
