...
const deletePaymentMethod = async () => {
  ...
  try {
    await paymentStore.deletePaymentMethod(user.username, deletingMethod.value.tbk_user, auth.token)
...
<template>
  ...
  <div :class="{ section: !isMobile, 'mobile-section': isMobile }">
    ...
    <div v-if="!loading" class="payment-methods-container">
      ...
      <div class="add-card-section">
        ...
        <button class="btn-purple" @click="addPaymentMethod">
          {{ t('add_card') }}
        </button>
      </div>
      ...
      <div v-if="paymentMethods.length > 0" class="card-list">
        ...
        <div v-for="method in paymentMethods" :key="method.payment_method_id" class="card-item">
          ...
          <button class="delete-btn" @click="confirmDeleteMethod(method)">
            {{ t('delete') }}
          </button>
        </div>
      </div>
      ...
      <AlertModal :show="showDeleteModal" @close="closeDeleteModal">
        ...
        <template #body>
          ...
          <div class="modal-buttons">
            <button class="btn-blue" @click="closeDeleteModal">{{ t('cancel') }}</button>
            <button class="btn-red" @click="deletePaymentMethod">{{ t('delete') }}</button>
          </div>
        </template>
      </AlertModal>
    </Teleport>
